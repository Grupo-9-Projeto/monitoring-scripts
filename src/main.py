...
if not os.path.exists('dados_maquina.csv'):
    with open('dados_maquina.csv', 'w', newline='') as csvfile:
        ...
        csv.writer(csvfile, delimiter=',').writerow(["cpu total(%)"] + cpus + [ "ram(%)", "disco(%)", "Quando foi Coletado", "Rede recebida(Mbps)", "Rede enviada(Mbps)", 'Frequencia de uso da CPU(MHz)', "Endereco MAC"])
